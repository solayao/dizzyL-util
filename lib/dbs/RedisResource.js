    ...
    function RedisResource() {
        ...
        var defaultOpt = {
            retry_strategy: function retry_strategy(options) {
                if (options.error && options.error.code === 'ETIMEDOUT') {
                    ErrorConsole('DBpool message', __filename, 'Connected MongoDB (' + options.error.address + ':' + options.error.port + ') NOT successfully to server!');
                }
        ...
        var connectFunc = function connectFunc() {
            ...
            redis.on("connect", function () {
                SuccessConsole('DBpool Message', __filename, 'Connected Reids (' + options.host + ':' + options.port + ') successfully to server.');
            });
            return redis;
        };
                    ...
                    while (1) {
                        switch (_context.prev = _context.next) {
                            ...
                            case 2:
                                ...
                                SuccessConsole('DBpool Message', __filename, 'Closed Reids (' + options.host + ':' + options.port + ') successfully to server.');
    ...
    _createClass(RedisResource, [{
        ...
        value: function lRange() {
            ...
            return this.action(function (client) {
                ...
                (_console = console).log.apply(_console, [key].concat(_toConsumableArray(length)));
